...
BEGIN
   ...
   -- log 1st step
   GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(vTopo,
   ...
   ELSE
       vNote := 'FAILED';
       vStatus := 'FAILED';
       GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(vTopo,vProcess,vTracking,
   ...
   -- CHECK 2: Look for bad Gtypes in all FSL tables in the topology
       ...
       GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(vTopo,vProcess,vTracking, p_step=>vStep,
                                     p_message=>vNote);
   ...
   vIsValid := 'FALSE';
   vIsValid := GZ_TOPO_UTIL.VALIDATE_TOPOLOGY(vtopo,vCount);
...
Begin
   ...
   If inpTbls IS NULL THEN
      ...
      LOOP
         ...
         GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(pTopo_IN,'FSL_BUILD', TrackingTable, p_message=>vNote);
  ...
  --If (vProject_Z9 = 'Y') Then
     ...
     -- drop any projection related tables (Topology || 'X')
      ...
      --         GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(pTopo_IN,'FSL_BUILD',TrackingTable,p_message=>SQL1);
      --         EXECUTE IMMEDIATE SQL1;
      --
      --      End Loop;
      --
...
Begin
   ...
   If rowCnt > 0 Then
      ...
      GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(pTopo_IN,'CLEANUP_FSLS',TrackingTable,p_message=>vNote);
...
Begin
   ...
   -- Already done in Calling Procedure (FSL_BUILD)
    ...
    GZ_BUSINESS_UTILS.GEN_FSL_TRACKING_LOG(pTopo_IN,'BUILD_FSLS',TrackingTable,p_message=>vNote);
