CREATE OR REPLACE PACKAGE BODY GZ_LINESIM AS
...
AS
  ...
  prei number;
Begin
  if GZ_SUPER.Table_Exists(skip_table) = FALSE then
    sql1 := 'CREATE TABLE ' ||skip_table||' (EDGE_ID NUMBER) NOLOGGING';
  execute immediate sql1;
  end if;
  sql1 := 'select face_id, island_edge_id_list from ' || tplgy1 || '_face$ ' ||
           ' where face_id in (select distinct a.right_face_id ' ||
                               ' from ' || tplgy1 || '_edge$ a, ' || tplgy2 || '_edge$ b ' ||
  ...
  Open cur1 for sql1;
  loop
     fetch cur1 into faceid, island_edge_list;
     exit when cur1%notfound;
     dbms_output.put_line('Faceid: ' || faceid);
     if island_edge_list.count > 0 then
     for i in island_edge_list.first..island_edge_list.last
     loop
         dbms_output.put_line('Island Edge List(' || i || '): ' || abs(island_edge_list(i)));
         sql1 := 'select abs(next_left_edge_id), abs(prev_left_edge_id), abs(next_right_edge_id),
abs(prev_right_edge_id) ' ||
                   '  from ' || tplgy1 || '_edge$ where edge_id = :1';
         execute immediate sql1 into nlei, plei, nrei, prei using abs(island_edge_list(i));
         dbms_output.put_line('Possible edges to eliminate: ' || nlei || ', ' || plei || ', ' || nrei || ', ' || prei);
 dbms_output.put_line('Edge to eliminate: ' ||  nrei );
        ...
        dbms_output.put_line ('Island Edge List: ' || island_edge_list.count);
     end if;
   end loop;
   close cur1;
   commit;
...
BEGIN
   ...
   GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', pJobRun,
                                                'START_LS_LOGGING', NULL,
...
PROCEDURE CREATE_GEN_LS_PARAMETERS (
   ...
   BEGIN
      ...
      psql := psql || ' NOPARALLEL NOLOGGING AS '
                   || 'SELECT * FROM TABLE(' || p_schema || '.GZ_LINESIM.NEW_LINE_SIM_PARAMETERS ) ';
...
BEGIN
   ...
   psql := 'CREATE TABLE ' || p_table_name || ' ';
   psql := psql || ' NOPARALLEL NOLOGGING AS '
                || 'SELECT * FROM TABLE(' || p_schema || '.GZ_LINESIM.NEW_GEN_LS_TRACKING ) ';
   BEGIN
      EXECUTE IMMEDIATE psql;
   EXCEPTION
      WHEN OTHERS
   ...
   BEGIN
     -- DBMS_OUTPUT.PUT_LINE('Begin Insert');
      ...
      THEN
         ...
         v_end_time := systimestamp;
      END IF;
      psql := 'INSERT /*+ APPEND */ INTO ' || p_jobrun || '_LS_TRACKING '
...
AS
   ...
   vProcess   VARCHAR2(100)   := 'Line Simplification ' || vProjectID || ' ' || vStateFP;
   vStepNum   NUMBER;
   vStep      VARCHAR2(4000);
...
BEGIN
   APP_INFO_MODULE := 'GZ_LINESIM.LINE_SIM-'||vSchema||','||vProjectID||','||vTopo;
   ...
   vLSModules := GZ_UTILITIES.split(vModules);
   IF vLSModules.COUNT <> 10
   THEN
      vNote := 'ERROR - wrong number of modules to run';
      dbms_output.put_line (vNote);
      RAISE_APPLICATION_ERROR(-20001,'Wrong number of modules (LSModules) passed');
   END IF;

   If vLSModules(3) = 'N' then
      vReRun := 'Y';
   else
      vReRun := 'N';
   end if;
...
*/
  ...
  if vReRun = 'N' then
      ...
      IF retval != '0' THEN
         RAISE_APPLICATION_ERROR(-20001,'Problem with START_LOGGING: ' || retval);
      ELSE
         DBMS_OUTPUT.PUT_LINE('Created Log Table ' || vTopo || '_LS_TRACKING');
      END IF;
  Else
     ...
     if rowCount = 0 then
        RAISE_APPLICATION_ERROR(-20001,'Tracking table must exist for a re-run');
     end if;
   ...
   vNote :=  'Input Parameters: '
                        ...
                        || ' pStateFP: ' || pStateFP
                        || ' pSkipEdgesTable: ' || pSkipEdgesTable 
                        || ' ptopofix_qa: ' || ptopofix_qa;
                        
   GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
   ...
   sql1 := 'SELECT target_scale, release FROM gz_job_setup WHERE jobid =   :1';
   ...
   then
      If vReRun = 'N' then
         vNote := 'Rerun flag is set to ' || vReRun || ' Topology ' || vTopo || ' exists.  Terminating execution';
         ...
         vNote := 'Rerun flag is set to ' || vReRun || ' Topology ' || vTopo || ' exists.  Continue execution';
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   if rowCount = 0
   then
      vNote := 'State Outline Table ' || pEntity_Table || ' does not exist.  Terminating execution';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   vNote := 'Now check for tmp tbls pEdges_table: ' ||  vTopo||pedges_table || ' pDone_edges_table: ' ||  vTopo||pDone_edges_table;
   GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                     p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

   sql1 := 'Select count(*) from user_tables where table_name in (:1, :2)';
   execute immediate sql1 into rowCount using vTopo||pedges_table,  vTopo||pDone_edges_table;
   if rowCount > 0
   then
      ...
      execute immediate sql1 bulk collect into table_arr using  vTopo||pedges_table,  vTopo||pDone_edges_table;
      ...
      vNote := vTopo||pedges_table || ' and ' ||  vTopo||pDone_edges_table || ' tables dont exist.  Need not drop them';
      dbms_output.put_line(vNote);
   end if;
   ...
   vStepNum := 10;
   vStep := 'Copy topology from ' || vSchema || '.' ||vSrcTopo || ' to ' ||vSchema || '.' || vTopo;
   If vLSModules(1) = 'Y'
   THEN
      ...
      gz_topo_util.copy_topology(vSchema,vSrcTopo,vSchema,vTopo,'Y','N');
      vNote := 'End '||vStep;
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   vInternal_Edge_Sql := 'select count(edge_id) from '||vEdgeDollarTable||' where left_face_id != -1 AND right_face_id != -1';
   EXECUTE IMMEDIATE vInternal_Edge_Sql INTO vInternal_Edge_Count;

   if vInternal_Edge_Count > 0 THEN
      vNote := vInternal_Edge_Count||' internal Edges were found, continuing with line simplification.';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

   else
      vNote := 'There are no internal edges in the '||vtopo||' topology.  Line Simplification will not change anything, so we can skip it.';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   end if;
   ...
   If vLSModules(3) = 'Y' and vInternal_Edge_Count > 0
   THEN
      DBMS_APPLICATION_INFO.SET_ACTION(vStepNum);
   ...
   dbms_output.put_line('--------------------Done with Step 30-------------------------------');
   --------------------------------------------------------------------------------
   ...
   THEN
      ...
      DBMS_OUTPUT.PUT_LINE('vQaEdgesTable: ' || vQaEdgesTable || 'vReportGeomColumn: ' || vReportGeomColumn || 'vEdgeDollarTable: ' || vEdgeDollarTable || 'vReportGeomColumn: ' || vReportGeomColumn || 'vQaReportTable: '  || vQaReportTable);
     ...
     DBMS_OUTPUT.PUT_LINE('GZ_QA.CHECK_ALL_VERTICES(' || vQaEdgesTable || ',' || vReportGeomColumn || ',' || vEdgeDollarTable || ',' || vReportGeomColumn || ','
                                                                                            || vQaReportTable  || ',' || vCAVMatchLength || ','
                                                                                            || vArea_Check || ',' || vPercent_Ungen || ',' || vMax_Consec_Match||  ',' || vCAVIncStEdges || ',' || vCAVPPrint || ');');


      GZ_QA.CHECK_ALL_VERTICES(vQaEdgesTable,  vReportGeomColumn, vEdgeDollarTable,
                                                   vReportGeomColumn, vQaReportTable, vCAVMatchLength,
                                                   vArea_Check, vPercent_Ungen, vMax_Consec_Match,
                                                   vCAVIncStEdges, vCAVPPrint);

      vNote := 'vQaReportTable: ' || vQaReportTable || ' vEdgeDollarTable: ' || vEdgeDollarTable;
      dbms_output.put_line(vNote);

      sql1 := 'Select count(*) from ' || vQaReportTable || '  where percent_ungen = 100';
      ...
      execute immediate sql1 into vUnGenPCT using vUnGenEdgeCount, vEdgeCount;

      vNote := 'UnGenEdgeCount = ' ||vUnGenEdgeCount || ' Total Edges = ' || vEdgeCount || ' unGenPCT = ' || vUnGenPCT || '  QA Tolerance = ' || vQaTolerance;
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

      If vUnGenPCT > vQaTolerance then
         vNote := 'Too many un generalized edges....  stop execution!';
          GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

         Success_Flag := 'N';
         QA_Note := QA_Note || ' Failed Step 40. Too many ungeneralized edges. ';
         --RAISE_APPLICATION_ERROR(-20001,vNote);

      else
         vNote := 'Un Generalized edges are within limit...  can continue to next step';
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
      ...
      IF vRes
        ...
        vNote := 'VALID: All Feature Tables are valid in '||vtopo;
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
      ELSE
        vNote := 'INVALID: Some Feature Table records are not valid in '||vtopo;
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
        RAISE_APPLICATION_ERROR(-20001,vNote);
      END IF;
   ...
   THEN
      ...
      --     'and a.ignore <> 1';
      vsql := 'select count(1) from '||vTopo||pedges_table||' a,'||
           vTopo||'_EDGE$ t '||
           'where sdo_util.getnumvertices(a.new_geometry) <> '||
           'sdo_util.getnumvertices(t.geometry) and a.edge_id=t.edge_id '||
           'and a.ignore <> 1';
   ...
   ELSE
      vNote := 'QA Step60 - BAD. Sidey''s SQL count query = '||
                   vCheckCount||'. This is an informational message only.  Pay more attention to other QA checks';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   IF vCheckCount <> 0 THEN
      -- log an error
       vNote := 'FAILED! Found a mismatch on l/r faces between '||
                   vSrcTopo||' and '||vTopo;
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   If vLSModules(8) = 'Y' and vInternal_Edge_Count > 0 THEN
      DBMS_APPLICATION_INFO.SET_ACTION(vStepNum);
      DBMS_APPLICATION_INFO.SET_CLIENT_INFO('Check and fix conflict edges');
      vNote := 'Begin '||vStep;
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

      edgefix_val := GZ_TOPOFIX.GZ_FIX_EDGE (pJobId ,
                                             vTopo,
                                             'LS',
                                             'Y',    --Yes, hold universal face
                                             0.05,
                                             NULL,   --No fixed loop count. Continue as long as theres progress
                                             'Y');   --yes check for close pairs of edges
      ...
      THEN
         ...
         vNote := 'Step '||vStepNum||' - Fail '|| vStep || ' but we wont fail LS for it ';
         GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                           p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   THEN
      ...
      GZ_UTILITIES.UPDATE_FACE_MEASUREMENTS(pface);
      vNote := 'End '||vStep;
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   If vLSModules(9) = 'Y' and vInternal_Edge_Count > 0
   THEN
      DBMS_APPLICATION_INFO.SET_ACTION(vStepNum);
      DBMS_APPLICATION_INFO.SET_CLIENT_INFO(vStep);
      vNote := 'Begin '||vStep;
   ...
   vOutput := GZ_TOPOFIX.check_face_tab(pJobId, vTopo,pFace,'FACE_ID', 'LS');
    If (vOutput = '0' ) THEN
      dbms_output.put_line('TRUE');
      vNote := 'PASS - '||vTopo||' ready for FSL build if steps 60 and 70 passed.';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

   ELSE
      dbms_output.put_line('FALSE');
      vNote := 'FAIL - in '||vTopo||' some faces didn''t validate.  Fix them before FSL build.';
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                        p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);

      Success_Flag := 'N';
      QA_Note := QA_Note || ' Failed Step 90. Unable to validate all faces. ';
   ...
   If vLSModules(10) = 'Y' and vInternal_Edge_Count > 0
   THEN
      DBMS_APPLICATION_INFO.SET_ACTION(vStepNum);
      ...
      Begin
        ...
        retVal := GZ_UTILITIES.VALIDATE_TOPOLOGY(vtopo,rowCount);
         ...
         If retVal <> 'TRUE' then
               dbms_output.put_line('Source Topology ' || vTopo || ' is not valid');
      ...
      Exception
         When Others Then
            QA_Note := QA_Note || ' Failed Step 100. Unable to Validate Topology';
            GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess,
                              p_step=>vStepNum, p_sqlstmt=>vStep, p_error_msg=>vNote);
   ...
   --END LOOP;
    If (Success_Flag = 'Y') then
       ...
       GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess, p_error_msg=>vNote);

    else
      GZ_UTILITIES.GEN_EXTENDED_TRACKING_LOG('LS', vTopo, vProcess, p_error_msg=>QA_Note);
...
END LINE_SIM;
   ...
   BEGIN
      psql := 'SELECT a.* FROM LINE_SIM_PARAMETERS a '
           || 'WHERE '
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20001,'Line_Sim_Parameter record not found for project id ' || p_project_id );
         WHEN TOO_MANY_ROWS THEN
            RAISE_APPLICATION_ERROR(-20001,'Found more than one Line_Sim_Parameter record for project id ' || p_project_id);
         WHEN OTHERS THEN
            IF SQLCODE = -942
            THEN
               RAISE_APPLICATION_ERROR(-20001,'Table LINE_SIM_PARAMETERS does not exist!');
            ELSE
               RAISE;
            END IF;
      END;
